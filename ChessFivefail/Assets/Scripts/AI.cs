...
class AI
{
    ...
    //玩家的可能性
	bool[,,] _ptable = new bool[BoardCross.CrossBorad, BoardCross.CrossBorad, MaxFiveChainCount];

    //电脑的可能性
	bool[, ,] _ctable = new bool[BoardCross.CrossBorad, BoardCross.CrossBorad, MaxFiveChainCount];

    //记录2位玩家所有可能的连珠数，-1则为永远无法5连珠
    int[,] _win = new int[2, MaxFiveChainCount];

    //记录每格的分值
	int[,] _cgrades = new int[BoardCross.CrossBorad, BoardCross.CrossBorad];
	int[,] _pgrades = new int[BoardCross.CrossBorad, BoardCross.CrossBorad];

    //记录棋盘
	int[,] _board = new int[BoardCross.CrossBorad, BoardCross.CrossBorad];
    ...
    public AI( )
    {
		for ( int i = 0;i<BoardCross.CrossBorad;i++)
        {
			for ( int j = 0;j<BoardCross.CrossBorad;j++)
            {
                _pgrades[i, j] = 0;
                _cgrades[i, j] = 0;
                _board[i, j] = 0;
		...
		for ( int i = 0;i<BoardCross.CrossBorad;i++)
         {
			for ( int j = 0;j<BoardCross.CrossBorad- 4 ;j++)
             {
		...
		for (int i = 0; i < BoardCross.CrossBorad; i++)
         ...
         // 右斜
		for (int i = 0; i < BoardCross.CrossBorad - 4; i++)
         {
			for (int j = 0; j <BoardCross.CrossBorad - 4; j++)
             {
                 for (int k = 0; k < BoardModel.WinChessCount; k++)
		...
		for (int i = 0; i <BoardCross.CrossBorad- 4; i++)
         {
			for (int j = BoardCross.CrossBorad- 1; j >= 4; j--)
             {
                 for (int k = 0; k < BoardModel.WinChessCount; k++)
                 {
                     _ptable[j - k, i + k, _icount] = true;
